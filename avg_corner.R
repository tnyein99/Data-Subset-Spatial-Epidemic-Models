...
avg_corner<-function(sam,pv){
  ...
  for(k in 1:length(pv)){
    ...
    xl_p1<-0
    xu_p1<-sqrt(sam*percent)
    yl_p1<-0
    yu_p1<-sqrt(sam*percent)
    ...
    #extracting indices of the y coordinates which are less than yl_p1 and yu_p1
    ind_vec_y_p1<-c()
    for(i in 1:nrow(sir_mod0$XYcoordinates)){
      test_vec_p1<-c(sir_mod0$XYcoordinates[i,])
    ...
    xl_p2<-sqrt(sam)-sqrt(sam*percent)
    xu_p2<-sqrt(sam)
    yl_p2<-0
    yu_p2<-sqrt(sam*percent)
    ...
    xl_p3<-0
    xu_p3<-sqrt(sam*percent)
    
    yl_p3<-sqrt(sam)-sqrt(sam*percent)
    yu_p3<-sqrt(sam)
    ...
    #extracting indices of the y coordinates which are less than yl_p3 and xu_p3
    ind_vec_y_p3<-c()
    for(i in 1:nrow(sir_mod0$XYcoordinates)){
      test_vec_p3<-c(sir_mod0$XYcoordinates[i,])
    ...
    xl_p4<-sqrt(sam)-sqrt(sam*percent)
    xu_p4<-sqrt(sam)
    
    yl_p4<-sqrt(sam)-sqrt(sam*percent)
    yu_p4<-sqrt(sam)
    ...
    #extracting indices of the y coordinates which are less than yl_p4 and xu_p4
    ind_vec_y_p4<-c()
    for(i in 1:nrow(sir_mod0$XYcoordinates)){
      test_vec_p4<-c(sir_mod0$XYcoordinates[i,])
    ...
    ##calculating corner parameter estimates
    alpha_sub[k]<-mean(c(sum_p1_spatial$statistics[1,1],sum_p2_spatial$statistics[1,1],
                         sum_p3_spatial$statistics[1,1],sum_p4_spatial$statistics[1,1]))
    
    alpha_sd_sub[k]<-mean(c(sum_p1_spatial$statistics[1,2],sum_p2_spatial$statistics[1,2],
                            sum_p3_spatial$statistics[1,2],sum_p4_spatial$statistics[1,2]))
    
    beta_sub[k]<-mean(c(sum_p1_spatial$statistics[2,1],sum_p2_spatial$statistics[2,1],
                        sum_p3_spatial$statistics[2,1],sum_p4_spatial$statistics[2,1]))
    
    beta_sd_sub[k]<-mean(c(sum_p1_spatial$statistics[2,2],sum_p2_spatial$statistics[2,2],
...
write.csv(df_abs_biases,"/home/thethtetchan.nyein/avg_corner_100.csv", row.names = FALSE)
