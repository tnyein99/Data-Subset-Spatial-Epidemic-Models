...
avg_random<-function(sam){
  ...
  ##randomness factor is incorporated
  
  repeat{
    xl_25p1<-runif(1,0,sqrt(sam))
    xu_25p1<-xl_25p1+sqrt(sam*0.25)
    ##make sure upper limit contains in the square grid
    if(xu_25p1<=sqrt(sam))
      break
  }
  
  repeat{
    yl_25p1<-runif(1,0,sqrt(sam))
    yu_25p1<-yl_25p1+sqrt(sam*0.25)
    ##make sure upper limit contains in the square grid
    if(yu_25p1<=sqrt(sam))
      break
  }
  ...
  mcmc_out25p1<-epimcmc(object = sir_25p1, tmin = min(inftime_25p1), tmax = max(inftime_25p1),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_25p1_spatial<-summary(mcmc_out25p1, start = 10001)
  
  
  ## subset 2
  
  repeat{
    xl_25p2<-runif(1,0,sqrt(sam))
    xu_25p2<-xl_25p2+sqrt(sam*0.25)
    ##make sure upper limit contains in the square grid
    if(xu_25p2<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_25p2<-runif(1,0,sqrt(sam))
    yu_25p2<-yl_25p2+sqrt(sam*0.25)
    ##make sure upper limit contains in the square grid
    if(yu_25p2<=sqrt(sam))
      break
  }
  ...
  #extracting indices of the x coordinates which are between yl_25p2 and yu_25p2
  ind_vec_y_25p2<-c()
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
    test_vec_25p2<-c(sir_mod0$XYcoordinates[i,])
  ...
  mcmc_out25p2<-epimcmc(object = sir_25p2, tmin = min(inftime_25p2), tmax = max(inftime_25p2),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_25p2_spatial<-summary(mcmc_out25p2, start = 10001)
  
  repeat{
    xl_25p3<-runif(1,0,sqrt(sam))
    xu_25p3<-xl_25p3+sqrt(sam*0.25)
    ##make sure upper limit contains in the square grid
    if(xu_25p3<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_25p3<-runif(1,0,sqrt(sam))
    yu_25p3<-yl_25p3+sqrt(sam*0.25)
    ##make sure upper limit contains in the square grid
    if(yu_25p3<=sqrt(sam))
      break
  }
  ...
  mcmc_out25p3<-epimcmc(object = sir_25p3, tmin = min(inftime_25p3), tmax = max(inftime_25p3),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_25p3_spatial<-summary(mcmc_out25p3, start = 10001)
  
  repeat{
    xl_25p4<-runif(1,0,sqrt(sam))
    xu_25p4<-xl_25p4+sqrt(sam*0.25)
    ##make sure upper limit contains in the square grid
    if(xu_25p4<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_25p4<-runif(1,0,sqrt(sam))
    yu_25p4<-yl_25p4+sqrt(sam*0.25)
    ##make sure upper limit contains in the square grid
    if(yu_25p4<=sqrt(sam))
      break
  }
  ...
  mcmc_out25p4<-epimcmc(object = sir_25p4, tmin = min(inftime_25p4), tmax = max(inftime_25p4),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_25p4_spatial<-summary(mcmc_out25p4, start = 10001)
  ...
  repeat{
    xl_36p1<-runif(1,0,sqrt(sam))
    xu_36p1<-xl_36p1+sqrt(sam*0.36)
    ##make sure upper limit contains in the square grid
    if(xu_36p1<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_36p1<-runif(1,0,sqrt(sam))
    yu_36p1<-yl_36p1+sqrt(sam*0.36)
    ##make sure upper limit contains in the square grid
    if(yu_36p1<=sqrt(sam))
      break
  }
  ...
  mcmc_out36p1<-epimcmc(object = sir_36p1, tmin = min(inftime_36p1), tmax = max(inftime_36p1),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_36p1_spatial<-summary(mcmc_out36p1, start = 10001)
  
  
  ## subset 2
  
  repeat{
    xl_36p2<-runif(1,0,sqrt(sam))
    xu_36p2<-xl_36p2+sqrt(sam*0.36)
    ##make sure upper limit contains in the square grid
    if(xu_36p2<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_36p2<-runif(1,0,sqrt(sam))
    yu_36p2<-yl_36p2+sqrt(sam*0.36)
    ##make sure upper limit contains in the square grid
    if(yu_36p2<=sqrt(sam))
      break
  }
  ...
  mcmc_out36p2<-epimcmc(object = sir_36p2, tmin = min(inftime_36p2), tmax = max(inftime_36p2),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_36p2_spatial<-summary(mcmc_out36p2, start = 10001)
  
  repeat{
    xl_36p3<-runif(1,0,sqrt(sam))
    xu_36p3<-xl_36p3+sqrt(sam*0.36)
    ##make sure upper limit contains in the square grid
    if(xu_36p3<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_36p3<-runif(1,0,sqrt(sam))
    yu_36p3<-yl_36p3+sqrt(sam*0.36)
    ##make sure upper limit contains in the square grid
    if(yu_36p3<=sqrt(sam))
      break
  }
  ...
  mcmc_out36p3<-epimcmc(object = sir_36p3, tmin = min(inftime_36p3), tmax = max(inftime_36p3),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_36p3_spatial<-summary(mcmc_out36p3, start = 10001)
  
  repeat{
    xl_36p4<-runif(1,0,sqrt(sam))
    xu_36p4<-xl_36p4+sqrt(sam*0.36)
    ##make sure upper limit contains in the square grid
    if(xu_36p4<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_36p4<-runif(1,0,sqrt(sam))
    yu_36p4<-yl_36p4+sqrt(sam*0.36)
    ##make sure upper limit contains in the square grid
    if(yu_36p4<=sqrt(sam))
      break
  }
  ...
  mcmc_out36p4<-epimcmc(object = sir_36p4, tmin = min(inftime_36p4), tmax = max(inftime_36p4),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_36p4_spatial<-summary(mcmc_out36p4, start = 10001)
  ...
  repeat{
    xl_49p1<-runif(1,0,sqrt(sam))
    xu_49p1<-xl_49p1+sqrt(sam*0.49)
    ##make sure upper limit contains in the square grid
    if(xu_49p1<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_49p1<-runif(1,0,sqrt(sam))
    yu_49p1<-yl_49p1+sqrt(sam*0.49)
    ##make sure upper limit contains in the square grid
    if(yu_49p1<=sqrt(sam))
      break
  }
  ...
  mcmc_out49p1<-epimcmc(object = sir_49p1, tmin = min(inftime_49p1), tmax = max(inftime_49p1),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_49p1_spatial<-summary(mcmc_out49p1, start = 10001)
  
  
  ## subset 2
  
  repeat{
    xl_49p2<-runif(1,0,sqrt(sam))
    xu_49p2<-xl_49p2+sqrt(sam*0.49)
    ##make sure upper limit contains in the square grid
    if(xu_49p2<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_49p2<-runif(1,0,sqrt(sam))
    yu_49p2<-yl_49p2+sqrt(sam*0.49)
    ##make sure upper limit contains in the square grid
    if(yu_49p2<=sqrt(sam))
      break
  }
  ...
  mcmc_out49p2<-epimcmc(object = sir_49p2, tmin = min(inftime_49p2), tmax = max(inftime_49p2),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_49p2_spatial<-summary(mcmc_out49p2, start = 10001)
  
  repeat{
    xl_49p3<-runif(1,0,sqrt(sam))
    xu_49p3<-xl_49p3+sqrt(sam*0.49)
    ##make sure upper limit contains in the square grid
    if(xu_49p3<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_49p3<-runif(1,0,sqrt(sam))
    yu_49p3<-yl_49p3+sqrt(sam*0.49)
    ##make sure upper limit contains in the square grid
    if(yu_49p3<=sqrt(sam))
      break
  }
  ...
  mcmc_out49p3<-epimcmc(object = sir_49p3, tmin = min(inftime_49p3), tmax = max(inftime_49p3),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_49p3_spatial<-summary(mcmc_out49p3, start = 10001)
  
  repeat{
    xl_49p4<-runif(1,0,sqrt(sam))
    xu_49p4<-xl_49p4+sqrt(sam*0.49)
    ##make sure upper limit contains in the square grid
    if(xu_49p4<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_49p4<-runif(1,0,sqrt(sam))
    yu_49p4<-yl_49p4+sqrt(sam*0.49)
    ##make sure upper limit contains in the square grid
    if(yu_49p4<=sqrt(sam))
      break
  }
  ...
  mcmc_out49p4<-epimcmc(object = sir_49p4, tmin = min(inftime_49p4), tmax = max(inftime_49p4),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_49p4_spatial<-summary(mcmc_out49p4, start = 10001)
  ...
  repeat{
    xl_64p1<-runif(1,0,sqrt(sam))
    xu_64p1<-xl_64p1+sqrt(sam*0.64)
    ##make sure upper limit contains in the square grid
    if(xu_64p1<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_64p1<-runif(1,0,sqrt(sam))
    yu_64p1<-yl_64p1+sqrt(sam*0.64)
    ##make sure upper limit contains in the square grid
    if(yu_64p1<=sqrt(sam))
      break
  }
  ...
  #extracting x_coordinates
  ind_vec_x_64p1<-c()
  #extracting indices of the x coordinates which are between xl_61p1 and xu_64p1
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
  ...
  ind_vec_y_64p1<-c()
  #extracting indices of the x coordinates which are between yl_61p1 and yu_64p1
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
    test_vec_64p1<-c(sir_mod0$XYcoordinates[i,])
  ...
  mcmc_out64p1<-epimcmc(object = sir_64p1, tmin = min(inftime_64p1), tmax = max(inftime_64p1),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_64p1_spatial<-summary(mcmc_out64p1, start = 10001)
  
  
  ## subset 2
  
  repeat{
    xl_64p2<-runif(1,0,sqrt(sam))
    xu_64p2<-xl_64p2+sqrt(sam*0.64)
    ##make sure upper limit contains in the square grid
    if(xu_64p2<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_64p2<-runif(1,0,sqrt(sam))
    yu_64p2<-yl_64p2+sqrt(sam*0.64)
    ##make sure upper limit contains in the square grid
    if(yu_64p2<=sqrt(sam))
      break
  }
  #extracting x_coordinates
  ind_vec_x_64p2<-c()
  #extracting indices of the x coordinates which are between xl_61p2 and xu_64p2
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
  ...
  #extracting indices of the x coordinates which are between yl_61p2 and yu_64p2
  ind_vec_y_64p2<-c()
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
    test_vec_64p2<-c(sir_mod0$XYcoordinates[i,])
  ...
  mcmc_out64p2<-epimcmc(object = sir_64p2, tmin = min(inftime_64p2), tmax = max(inftime_64p2),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_64p2_spatial<-summary(mcmc_out64p2, start = 10001)
  
  repeat{
    xl_64p3<-runif(1,0,sqrt(sam))
    xu_64p3<-xl_64p3+sqrt(sam*0.64)
    ##make sure upper limit contains in the square grid
    if(xu_64p3<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_64p3<-runif(1,0,sqrt(sam))
    yu_64p3<-yl_64p3+sqrt(sam*0.64)
    ##make sure upper limit contains in the square grid
    if(yu_64p3<=sqrt(sam))
      break
  }
  ...
  #extracting x_coordinates
  ind_vec_x_64p3<-c()
  #extracting indices of the x coordinates which are between xl_61p3 and xu_64p3
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
  ...
  #extracting indices of the x coordinates which are between yl_61p3 and yu_64p3
  ind_vec_y_64p3<-c()
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
    test_vec_64p3<-c(sir_mod0$XYcoordinates[i,])
  ...
  mcmc_out64p3<-epimcmc(object = sir_64p3, tmin = min(inftime_64p3), tmax = max(inftime_64p3),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_64p3_spatial<-summary(mcmc_out64p3, start = 10001)
  
  repeat{
    xl_64p4<-runif(1,0,sqrt(sam))
    xu_64p4<-xl_64p4+sqrt(sam*0.64)
    ##make sure upper limit contains in the square grid
    if(xu_64p4<=sqrt(sam))
      break
  }
  ...
  repeat{
    yl_64p4<-runif(1,0,sqrt(sam))
    yu_64p4<-yl_64p4+sqrt(sam*0.64)
    ##make sure upper limit contains in the square grid
    if(yu_64p4<=sqrt(sam))
      break
  }
  ...
  #extracting x_coordinates
  ind_vec_x_64p4<-c()
  #extracting indices of the x coordinates which are between xl_61p4 and xu_64p4
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
  ...
  #extracting indices of the x coordinates which are between yl_61p4 and yu_64p4
  ind_vec_y_64p4<-c()
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
    test_vec_64p4<-c(sir_mod0$XYcoordinates[i,])
  ...
  mcmc_out64p4<-epimcmc(object = sir_64p4, tmin = min(inftime_64p4), tmax = max(inftime_64p4),
                        niter = 50000, sus.par.ini = 1, beta.ini = 1,
                        Sformula = NULL, pro.sus.var = 0.3, pro.beta.var = 0.1,
                        prior.sus.dist = "gamma", prior.beta.dist = "gamma",
                        prior.sus.par = alphapar2, prior.beta.par = betapar2,
                        adapt = FALSE, acc.rate = NULL)
  sum_64p4_spatial<-summary(mcmc_out64p4, start = 10001)
...
write.csv(df_abs_biases,"/home/thethtetchan.nyein/avg_random_100_abs_biases.csv", row.names = FALSE)
