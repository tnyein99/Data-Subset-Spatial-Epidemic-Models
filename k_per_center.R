...
k_per_center<-function(sam,percent){
  ...
  ind_vec_x_p<-c()
  #extracting indices of the x coordinates which are less than between xl_p and xu_p
  ...
  #extracting indices of the y coordinates which are less than between yl_p and yu_p
  ind_vec_y_p<-c()
  for(i in 1:nrow(sir_mod0$XYcoordinates)){
    test_vec_p<-c(sir_mod0$XYcoordinates[i,])
...
write.csv(df_abs_biases,"/home/thethtetchan.nyein/percent10_100_center_abs_biases.csv", row.names = FALSE)
